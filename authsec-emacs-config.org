...
* Introduction
  ...
  I am using the =emacs-mac= package from =brew=. It is installed
  with:
  ...
  In order to be able to open Emacs from a Spotlight search you need to create a proper alias. A symlink won't show up in Spotlight.

  You can create the proper alias with the following command:
